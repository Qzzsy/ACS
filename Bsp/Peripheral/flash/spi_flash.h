...
{
    struct rt_device                flash_device; 	/* rtthread 设备对象属性 */
    struct rt_device_blk_geometry   geometry; 		/* 设备的内存属性 */
    struct rt_spi_device *          rt_spi_device;	/* 设备所在的总线属性 */
    struct rt_mutex                 lock;			/* 互斥信号锁 */
    void *                          user_data;		/* 用户数据 */
};
